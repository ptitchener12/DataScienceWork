...
The first output is the mean of the entire data set.
The second outcome is the mean price of dimaonds grouped by color and clarity
...
#df_q4 <- 
diamonds %>% 
  mutate(depth_computed = 2 * z / (x + y)*100) %>% 
  mutate(diff = depth - depth_computed) %>% 
  summarise(cov = sd(x) / mean(x))
```
