...
df_q4 <- merge(df_covid, df_q3, by = c("fips")) #all.y = TRUE)
...
institutional_normalized <- institutional_covid %>% 
  ...
  mutate(nursing_home_perk = nursing_home / popk, college_perk = college_housing / popk, military_perk = military_housing / popk)
...
max_cases <- institutional_normalized %>% 
  ...
  summarise(
    ...
    military_housing = max(military_housing),
